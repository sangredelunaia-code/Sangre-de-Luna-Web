/* SANGRE DE LUNA · CRONISTA GLOBAL + PUENTE TOUR 360
   Conserva el Cronista estable, activa Tour, recuperación, bienvenida admin, insignias, expediciones, progreso, ranking, ruta guiada y tarjetas compartibles. */
(()=>{
  ...
  const loadTour=()=>{if(!isTour)return;addTourScrollbarFix();observeTourCTA();if(window.__SDL_TOUR360_MANAGER__||document.querySelector('script[data-sdl360-manager]'))return;const s=document.createElement('script');s.src=`${CDN}/tour360-manager.js?v=20260816`;s.defer=true;s.dataset.sdl360Manager='1';s.onload=repairTourCTA;document.head.appendChild(s)};
  ...
  const loadWelcomeAdmin=()=>{if(window.__SDL_FAN_ADMIN_WELCOME__||document.querySelector('script[data-sdl-admin-welcome]'))return;const s=document.createElement('script');s.src=`${CDN}/fanclub-admin-welcome.js?v=20260817-1`;s.defer=true;s.dataset.sdlAdminWelcome='1';document.head.appendChild(s)};
