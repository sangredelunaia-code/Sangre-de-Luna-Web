...
(()=>{
  ...
  const repairTourCTA=()=>{
    if(!isTour)return;
    const legacyStart=document.getElementById('startButton');if(legacyStart&&!legacyStart.textContent.trim())legacyStart.textContent='Explorar La Ciudadela';
    document.querySelectorAll('#sdl360App .s360-btn.primary,#sdl360App .s360-start,#sdl360App button[data-action="start"]').forEach(button=>{button.style.setProperty('color','#07101a','important');button.style.setProperty('-webkit-text-fill-color','#07101a','important');if(!button.textContent.trim())button.textContent='Explorar ahora'});
  };
